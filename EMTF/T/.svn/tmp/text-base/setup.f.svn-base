...
c ME		to SETUP. See comments marked ME below
     ...
     &  decl,orient,chid,dr,npts,nchstk,ncht) !,isuse,nuset
      ...
      character*40 stnames(nstamx,ntpmax),outname,fname,cfpcoh
      character*40 cfile,cdirin,cdirout,cbandf !,cdircf
      character*80 chead
...
ccc       get channel IDs, orientations etc.
     ...
     &         'ndmax =',ndmax
         ...
         if(nfreqmxi.gt.nfreqmax) then
            print*,'error: nfreqmax in file header exceeds max set',
     &       ' in program'
            print*,'file = ', fname,'nfreqmxi =',nfreqmxi,
     &               '  nfreqmx = ',nfreqmax
         end if
      enddo     ! i = 1,nsta
