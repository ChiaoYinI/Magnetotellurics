      subroutine rbstSDM(xx,nf,nchstk,s,nu,ru,tol,lrbst,w,
cRefOut Begin Changes
     &   wref,
cRefOut End Changes
     & nchin,lref,iter,idf)

cRefOut Begin Changes
ccc   April 6, 1997 ... Egbert, modifications to protect against
ccc   very large outliers in remote ...
ccc    lines marked with cRefOut
cRefOut End Changes
...
cRefOut   Begin Chages 
      if(lref) then
ccc      iref is channel # for first reference channel
         iref = nch+1
         call edfwtsRR(xx,nchin,iref,nf,p1,p2,p3,h,w)
      else
         call edfwts(xx,nchin,nf,p1,p2,h,w)
      endif

c      if(lref) then
ccc      compute a similar set of weights for the remote 
ccc      (1) estimate horizontal mag field sdm robustly
c         call mkrhat(edfl1,nf,xx(nch+1,1),nchin,h,ierr,wref)
c         if(ierr .eq. -1) then
c            print*,'error in mkrhat'
c            return
c         end if

ccc     (2) compute weights for reducing effect of reference outliers
c         call edfwts(xx(nch+1,1),nchin,nf,p1,p2,h,wref)

ccc      multiply by leverage weights
c         do i = 1,nf
c            w(i) = w(i)*wref(i)
c         enddo
c       endif
