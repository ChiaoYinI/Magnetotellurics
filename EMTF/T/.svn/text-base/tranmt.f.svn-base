...
c              print*, nuset, (isuse(1,1,ka),isuse(2,1,ka), ka = 1,nuset)
            elseif(arg(1:2).eq.'-b') then
               lBadRec = .true.
               read(arg(3:80),'(a80)') badRecFile
           endif
        enddo
      write(0,*) 'l_GRP = ',l_GRP
...
c.... set up for frequency band averaging
     ...
     &   dr,npts,cbandf)   !,isuse,nuset
      ...
      if(lBadRec) then
ccc      set isuse using info from bad record file
         brUnit = 88
         fac = 0.75
         nduset = idl(nbt)
         call badToUse(badRecFile,brUnit,ndMax,nduset,dr,nuset,isuse,
     &      npts,fac)
c         write(0,*) 'Omitting'
c         do i = 1,nuset-1
c            do k = 1, nduset
c                write(0,*) isuse(2,k,i)+1,' - ',isuse(1,k,i+1)-1,
c     &                ' at level ',k
c            enddo
c         enddo
         do k = 1,nduset
            isuse(1,k,1) = -maxint
            isuse(2,k,nuset) = maxint
         enddo
      else
         ...
         do i = 1, nuset
            do k = 2, nduset
               ...
               isuse(1,k,i) = int((isuse(1,k-1,i)-1)/dec_fac+.999)+1
               isuse(2,k,i) = int((isuse(2,k-1,i)-1)/dec_fac) + 1
            enddo
         enddo 
...
c         if (isuse(1,1,1).ne.-maxint .and. isuse(2,1,1).ne.maxint) then
 ...
 894     format (/,'***********************************************',/)
 895     format (/,'                 PROCESSING ALL SETS  ',/)
      endif
...
c	ME	nuset added to pass to mkrec
     ...
     &         xx,lx,ixs,ixf,xo,iwrk,l_PRISET)
         ...
         if(nfreq .le. 2 ) then
ccc          Don't process this band, AND
ccc          stop processing of further bands
ccc           (assuming these will all have too few
ccc            frequencies also)
             nbt = ib - 1
             goto 110
         endif
...
c..........coherence presort
     ...
     &       cohp,nu,coh,lpcoh,cfpcoh,xo,period(ib),w,ntfmax)
         ...
         if(lcoh) then
            call rbstSDM(xo,nu,nchstk,z(1,ib),ldf(ib),rdf(1,ib),tol
     &      ,lrbst,w,wref,ncht,lrref,iiter,idf(ib))
         else
            call rbstSDM(xx,nfreq,nchstk,z(1,ib),ldf(ib),rdf(1,ib),tol
     &      ,lrbst,w,wref,ncht,lrref,iiter,idf(ib))
         end if
         write(*,'(i4,i7,f12.4,i5,3x,i5,3x,i8,i8,i6,i8)') ib,idl(ib),
...
c      write(*,'(10a6)') (chid(k),k=1,ncht)
     ...
     +   stcor,decl,chid,dr,ndmax,chead,ntmx,l_SDM,bw,idf)
