...
c      nch = number of data channels
      ...
      integer ix(0:nch,*),ierr,nch,msval,msval_asc
...
ccc      read in consecutive data records with no sample numbers
         if(cform(1:1).eq.'*') then
            do j = 1,npts
               ...
               do k = 1,nch
                   if(ix(k,j) .eq. msval_asc) ix(k,j) = msval
               enddo
               ix(0,j) = irec
               irec = irec + 1
            enddo
...
ccc_____________________________________________________________________
ccc
      subroutine nimsInit(nch)

ccc   nimsInit  reads in clock reset information (time of zero record +
ccc    time of universal clock zero), and sets up sample numbering for
ccc    NIMS ascii data files
...
ccc   read until first non-header line
      nHead = 0
100   continue
      read(in_unit,'(a256)') cline
...
ccc      back up to start of clock info
         backspace(in_unit)
         backspace(in_unit)
         backspace(in_unit)
         backspace(in_unit)
...
ccc          [ hour,min,sec] (i.e., the time of the first record in the file)
         read(in_unit,'(a256)') cline
         read(cline(2:50),*) iyr,imo,iday,ihr,imin,isec
         write(0,*) 'date = ',iyr,imo,iday,ihr,imin,isec
ccc      UNIVERSAL CLOCK ZERO TIME - year month, day, time (ut) 
ccc          [ hour,min,sec] ( i.e., reference time for synchronizing stations)
         read(in_unit,'(a256)') cline
         read(cline(2:50),*) iyru,imou,idayu,ihru,iminu,isecu
      endif
