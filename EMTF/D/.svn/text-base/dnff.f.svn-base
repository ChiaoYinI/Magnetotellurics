     ...
     &  nmax = 4*nwmx,nwsvmx = nwmx*4+15,nfmax=nwmx/2,nwmx1=nwmx+10
     ...
     &  ,lfd(nchmx,ndmx),lwinst(ndmx),lend
      ...
      character*256 afparam(nfilmax,nchmx)
...
ccc         NIMS ascii data file, read clock reset info from header of data file
            l_asc = .true.
            l_NIMS = .true.
            l_clk_hd = .true.
         elseif(arg(1:2) .eq. '-B') then
ccc         NIMS binary data file, read clock reset info from header of data file
            l_asc = .false.
            l_NIMS = .true.
         endif
      enddo
      ...
      if(l_asc) then
         if(l_NIMS) then
            write(6,*) '  Reading from ASCII NIMS data file'
         else
            write(6,*) '  Reading from generic ASCII data file'
         endif
         write(6,*)
         if(l_clk_hd) then
     ...
     &                  '.clk file'
         ...
         if(l_NIMS) then
            write(6,*) '  Reading from NIMS binary data files'
         else
...
c       local file paths; open input file, make sp br etc path/file names
      ...
      if(ierr_bin_read .eq. -1) then
         write(6,*) 'Error Reading Binary File: ', err_file
         go to 500
      endif
...
c    file also gives offset to adjust record numbers with
        call bdrcsu(nd,nbadmx,ndp,idp,nds,ids,ioff,samprate(1),
     &     time_int)
ccccEGBERT
...
c    and reset pointers
      ...
      if(l_asc) then
         ...
         if(l_NIMS) then
            call rdblkNIMSbin(npts,ix1,ierr,nch)
         else
...
c              print*, 'good',irec,lrec,id,ist
              ...
              print*,'bad',lg,lgood,irec,lrec,id,ist
           endif
190     continue
        endif
200     continue
