...
implicit none
    ...
    integer (kind=4),parameter	:: nfil8 = 7 
    integer (kind=4),parameter	:: nfil4 = 3 
    integer (kind=4)		:: npts,msval,nHdrBytes,nHdrRec
    integer (kind=4)		:: nIn,nOut,jj,j,k,j1,j2
    ...
    if(iargc() .eq. 1) then
       ...
       j = irlong(cFileIn)
       !  add .bin to root for input
       cFileIn = cFileIn(1:j)//'.bin'
       !  add .dbn to root for decimated output
       cFileOut = cFileIn(1:j)//'.dbn'
