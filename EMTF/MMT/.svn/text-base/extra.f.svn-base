     ...
     & nuset,xx,wrk,iwrk,cwrk,nxu,la,ivunits,arrayid,sta,ixs,ixf,
     &  stc_tot,chid,decl_tot,orient_tot,nbt,nsig)

      include 'nstamx.inc'
      include '../include/four_byte.inc'
      integer nsta,nt_tot,nch(nsta),ntape(nsta),ih_tot(*),
     &    iband(2),iwrk(*),isuse(*),na(100),ivunits(*),ih(ntmx)
   
      character*4 sta(*),stau(nstamx)
...
ccc   Make array data records
      call mkrec(nd,id,iband,nsta,ntape,nch,nt_tot,isuse,nuset,
...
c        irecl =  4*(2+nt+2*ns)
         irecl = 4*(5+9*nstau+8*nt)

c         write(*,*)'irecl_was,irecl_my = ',4*(2+nt+2*ns),
c     *                                     4*(5+9*nstau+8*nt)
         irecl=max(irecl,4*(2+nt+2*ns))
...
c         write(*,*)irecl,nbt,nt,nstau,nsig,
c     &      (nchu(k),ih(k),stcor(1,k),stcor(2,k),decl(k),sta(k),
c     &      k=1,nstau),(orient(1,l),orient(2,l),chid(l),l=1,nt)
c
...
ccc      now move them from array x to array y
         npoint(ix) = ipoint + 1
c         print*,'nf(ix)',nf(ix),';  nt(ix)',nt(ix)
         id = 0
         do i = 1,nf_tot
            if(luse(i)) then
ccc            move data  
