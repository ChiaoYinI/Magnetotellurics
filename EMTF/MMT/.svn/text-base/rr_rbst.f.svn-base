...
ccc    ==> XP is work space iterations
...
ccc      complex xr(nd,nt),xc(nd,nt),xp(nd,nt),pctf(2,0:ntmx)
      real var(nt),eval(ntmx),orient(*),decl(*),theta,pwrmin
...
ccc   if requested by setting -M  option, omit all FCs for which net SNR
ccc   does not meet minimum threshold
c      write(0,*) 'PWRMIN   == ',pwrmin
...
c      write(0,*)work(iw2),work(iw3),work(iw5)
c      write(0,*)nd,npc,nt,itmax_ln,icvg,nomit
      call rbstreg(work(iw2),xr,xp,nd,npc,nt,pctf,work(iw3),
     &          work(iw5),se,xxinv,itmax_ln,icvg,nomit)
      if(icvg.lt.0) then
         write(0,*) 'in RR_RBST: did not converge'
         write(0,*) 'icvg= ',icvg
      else
         write(0,*) '# of iterations = ',icvg
      endif
