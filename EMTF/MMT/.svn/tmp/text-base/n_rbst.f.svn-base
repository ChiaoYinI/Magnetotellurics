...
ccc      ioux gives output unit for residuals (if >0 )
      ...
      real sndiag(ntmx),tf_se(ntmx,ntmx),sumSndiag
...
c       write(*,*) 'n_grp',n_grp
c       write(*,*) 'ih_grp',(ih_grp(i),i=1,n_grp)
      do i=1,n_grp
      ...
      sumSndiag = 0.
      do i = 1,nt
         sumSndiag = sumSndiag+abs(sndiag(i)) 
      enddo
      do i = 1,nt
         if (sndiag(i) .eq. 0) then
            sndiag(i) = sumSndiag/(100*nt)
         endif
      enddo
