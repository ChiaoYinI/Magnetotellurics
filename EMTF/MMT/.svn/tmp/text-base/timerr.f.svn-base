     ...
     &    lx,isuse,nuset,iterrb,nterrb,terr,sig,u,wrk,cwrk,iwrk,s)

c      Computes timing error corrections :  New version 12-10-93
c        IF twin is length of window in seconds,
c       timerr is time shift DT in seconds print

      include 'iosize.inc'

      parameter (pi = 3.14159, pi2 = pi/2.)

      integer iterrb(3,nterrb),isuse(*),nch(nsta),ntape(nsta),ih(*),
     & iband(2),iwrk(*),ie(*),orient(2,nt),nuset
...
ccc         get only data with all stations
         llx = .false.
         call mkrec(nd,id,iband,nsta,ntape,nch,nt,isuse,nuset,
...
c         print*,'terr',terr(ista)
c         print*,'sig',sig(ista)
